...
if __name__ == '__main__':
    ...
    # midlevel
    with open("../data/M4A/train-val-test/extras/midlevel.txt", "r", encoding="utf-8") as file:
        dnn = file.read().splitlines()  # Removes the newline characters

    # harmonic
    with open("../data/M4A/train-val-test/extras/harmonic.txt", "r", encoding="utf-8") as file:
        symb = file.read().splitlines()  # Removes the newline characters

    # signal
    with open("../data/M4A/train-val-test/extras/signal.txt", "r", encoding="utf-8") as file:
        sign = file.read().splitlines()  # Removes the newline characters

    # lyrical
    with open("../data/M4A/train-val-test/extras/lyrical.txt", "r", encoding="utf-8") as file:
        lyrical = file.read().splitlines()  # Removes the newline characters

    # labels
    with open("../data/M4A/train-val-test/extras/labels.txt", "r", encoding="utf-8") as file:
        labels = file.read().splitlines()  # Removes the newline characters

    # Read Splits
    tabular_path = '../data/M4A/train-val-test/tabular/'
    ...
    X_val = X_val.loc[:, ~X_val.columns.duplicated(keep="first")]
    ...
    X_val.drop(columns=categorical_cols+inf_cols, inplace=True)
    ...
    df_all = pd.read_pickle('../data/M4A/train-val-test/extras/df.pkl')
